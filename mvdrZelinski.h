/*
 * @Author: yehongcen 
 * @Date: 2023-02-05 21:12:26 
 * @Last Modified by:   yehongcen 
 * @Last Modified time: 2023-02-05 21:12:26 
 */
...
public:
    ...
    void DoBeamformimg(const float *data, int num_sample, bool is_noise,
        ...
        float vv2;
        {
            ...
            for (int m = 0; m < num_channel_ - 1; m++)
            {
                for (int n = m + 1; n < num_channel_; n++)
                {
                    for (int i = 0; i < num_valid_point_; i++)
                    {
                        ...
                        fenzi[i] += Pij[m][n][i] * 2 / (num_channel_ * (num_channel_ - 1));
                    }
                }
            }
            ...
            for(int i = 0;i < num_valid_point_;i++)
            {
                h[i] = fenzi[i] / (fenmu[i] + 0.0001);//Zelinski后置滤波
                //if(h[i] < 0) h[i] = -h[i];
            }
            for (int i = 0; i < num_valid_point_; i++) {
                fft_real[i] *= h[i];
